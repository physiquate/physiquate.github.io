document.addEventListener('DOMContentLoaded', function() {
  ...
  // ---------- MathJax helper (safe, can be used anywhere) ----------
  function renderMath(rootEl) {
    if (window.MathJax && MathJax.typesetPromise) {
      const args = rootEl ? [rootEl] : undefined; // only inside element if given
      MathJax.typesetPromise(args)
        .catch((err) => {
  ...
  (async function init() {
    ...
    createQuestionNodes();
    renderMath(document.getElementById('questions-root')); // typeset questions
    updateProgress();
    ...
    questionsArray.forEach((item, index) => {
      ...
      item.options.forEach((opt, i) => {
        ...
        input.addEventListener('change', () => {
          ...
          renderMath(document.getElementById('questions-root'));
  ...
  function submitExam() {
    ...
    // Typeset only inside the result section
    const resultBlock = document.getElementById('result');
    renderMath(resultBlock);
